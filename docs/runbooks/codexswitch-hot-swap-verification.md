...
- A verified ready result or disabling the monitor clears the incident claim and any matching delivered or pending notification. If a new incident begins while cancellation of the prior request is still completing, retain one replacement request and submit it only after late-delivery cleanup finishes. A failed enqueue clears the provisional claim so the next readiness observation can retry; repeated unhealthy observations are therefore safe retry opportunities, not additional notifications.
