...
- [ ] The bundled `patch-asar.py` hash matches the source used for the CodexSwitch build, modern ESM `@electron/asar` and legacy CommonJS `asar` fixtures produce the same Electron header-hash contract without a network install, the desktop-patch log records Fast fallback application without a structure warning, and the ASAR integrity hash plus strict code-sign verification pass before relaunch.
