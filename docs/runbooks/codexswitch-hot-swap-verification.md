...
- [ ] The installed ASAR contains `CODEXSWITCH_AUTH_CACHE_INVALIDATION_V3` and `CODEXSWITCH_AUTH_TRANSITION_V1` exactly once each, contains no nested legacy WeakMap helper, and a same-account auth notification produces no `_invalidateAccountQueries is not defined`, provider unmount, route remount, or window reload.
