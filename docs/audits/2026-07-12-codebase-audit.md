...
| VPS authority could publish a quota-red or credential-expired rotation target | Mac correctly adopted the authority target, but the selected account was already denied/zero-capacity or carried an expired inference generation; a green quota response could erase the token block | Swift and Rust now share fail-closed eligibility for fresh non-red quota plus an inference JWT beyond a five-minute safety window; fresh red and green-plus-expired adversarial rotations are rejected before authority publication |
| Current token-expired accounts retried provider refresh on every daemon pass | Active and inactive rejected refresh tokens were submitted repeatedly, producing a tight provider-I/O and log loop without creating a usable generation | Current token-expired blocks suppress quota, reset-bank, and token-refresh provider I/O until expiry or credential-generation replacement; successful quota telemetry no longer clears non-quota runtime blocks |
| A later manual switch could orphan a non-reset runtime rotation | The old VPS operation remained `started`, so a later automatic request with different semantics was rejected before candidate selection and an interrupted CLI could remain stopped even though authority was already stable on a usable account | A newer stable authority request now atomically resolves only an older non-reset operation to the stable target; reset-capable operations remain fail-closed, and replay tests cover same-ID adoption plus admission of the next operation |
